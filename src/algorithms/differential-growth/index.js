...
export default {
  longDescription: {
    ko: String.raw`
**Differential Growth**(차등 성장)는 뇌산호의 주름, 나뭇잎 가장자리의 구불거림, 꽃 잎사귀의 파형 등 자연에서 볼 수 있는 **불규칙한 공간 채우기** 현상을 수학적으로 모델링하는 알고리즘입니다. 선을 이루는 점(노드)들이 **서로 당기는 힘**(응집로 Cohesion)과 **밀어내는 힘**(반발력 Separation) 사이의 균형을 유지하면서, 인접 노드 간 거리가 임계값을 넘으면 자동으로 새 노드를 분열시키는 과정을 수행합니다. 이러한 과정을 통해 곡선은 전체적으로 **주름이 잡히며 유기적으로 성장**하고, 정교하고 우아한 기하학적 형태를 보여줍니다.
...
- **유기적인 주름:** 규칙적이지 않으면서도 조화로운 곡선의 주름이 깊이감과 복잡성을 부여합니다.
