...
export default {
  longDescription: {
    ko: String.raw`
**Stippling(스티플링)**은 이미지의 **명암 정보(Luminance)**를 **점의 밀도(Dot Density)**만으로 재현하는 전통적 예술 기법이자 현대 컴퓨터 그래픽 알고리즘입니다. 균일한 크기의 작은 점들을 **거리 기반 최적화(Distance-Based Optimization)**로 배치하면, 밝은 영역은 점들이 듬성듬성하고 어두운 영역은 조밀하게 모여 전체 이미지의 톤이 자연스럽게 복원됩니다. **Lloyd 완화(Lloyd Relaxation)** 또는 **Voronoi 다이어그램** 기반의 공간 최적화를 통해, 점들이 서서히 평형 위치에 정착하며 초상화가 드러나는 과정 자체가 이 알고리즘의 시각적 미학입니다.
