...
export default {
  longDescription: {
    ko: String.raw`
**Maze Generation(미로 생성)**은 **깊이 우선 탐색(Depth-First Search, DFS)** 기반의 **백트래킹(Backtracking)** 알고리즘으로, 격자 공간의 벽을 무작위로 허물며 **완전 연결 미로(Perfect Maze, 즉 시작에서 끝까지 유일한 경로가 존재하는 미로)**를 생성하는 **절차적 생성 알고리즘**입니다. 현재 셀에서 **방문하지 않은 이웃 셀**을 무작위로 선택하여 그 사이의 벽을 제거하고, 경로를 스택에 저장합니다. 막다른 길에 도달하면 스택을 역추적하여 새로운 분기를 탐색하며, 이 과정을 시각화하면 미로가 마치 살아있는 생명체처럼 천천히 펼쳐지는 모습을 관찰할 수 있습니다.
