...
export default {
  longDescription: {
    ko: String.raw`
**Chladni Figures**(클라드니 도형)은 진동하는 판의 고유 진동 모드(Eigenmode)를 시각화하는 알고리즘으로, Cymatics(음향 기하학)라는 현상을 보여주는 고전적이면서도 아름다운 수학 예술입니다. 1787년 독일의 물리학자 Ernst Chladni가 **진동하는 금속판 위에 모래를 뿌렸을 때 규칙적인 패턴**이 나타나는 현상을 발견했습니다. 이를 수학적으로 모델링하면 두 주파수 매개변수 $(n, m)$의 코사인 조합으로 정의된 진동 진폭 필드에서 진동이 정지하는 노드(Node)를 따라 입자가 모이는 것을 볼 수 있습니다.
...
### 2. 주요 특징 및 장점
- **물리와 수학의 우아함:** 미분방정식의 해가 시각적으로 표현되어, 추상적인 수학이 구체적인 기하학 패턴으로 드러나는 감동을 제공합니다.
...
- **정확성과 예술성의 조화:** 실제 물리 현상을 정확히 모델링하면서도 극도의 미적 결과를 얻습니다.
