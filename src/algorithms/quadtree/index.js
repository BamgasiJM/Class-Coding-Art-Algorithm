...
export default {
  longDescription: {
    ko: String.raw`
**Quadtree**(쿼드트리)는 2차원 평면을 **재귀적으로 4개의 하위 영역**으로 분할하는 **공간 분할 자료구조**입니다. 각 노드는 **정해진 용량** 만큼의 점을 보관하다가, 초과하면 자동으로 **4개의 자식 노드**로 분할됩니다. 이를 통해 2D 공간에서 점들을 계층적으로 정렬하여, **빠른 공간 탐색(Range Query)**, **충돌 판정(Collision Detection)**, **이미지 압축** 등에서 $O(\log n)$ 이상의 효율성을 달성합니다. 게임 엔진, 그래픽 라이브러리, 과학 시뮬레이션 등 실시간 시스템에서 필수적인 최적화 기법입니다.
