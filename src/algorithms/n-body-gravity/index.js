...
export default {
  longDescription: {
    ko: String.raw`
**N-Body Gravity(N체 중력)**는 우주의 모든 입자가 서로 **뉴턴의 만유인력(Universal Gravitation)**으로 상호작용하는 역학계를 시뮬레이션하는 **고전 천체역학** 알고리즘입니다. 각 입자 쌍 사이에 작용하는 인력은 **역제곱 법칙(Inverse-Square Law)** $F = \frac{G m_1 m_2}{r^2}$을 따르며, 이를 **시간 적분(Time Integration)**하여 입자들의 궤도 운동과 장기 동역학을 계산합니다. 중앙에 거대한 질량(항성)이 있고 그 주변을 작은 질량들(행성)이 공전하는 단순한 초기 조건에서도, 입자 간 상호작용으로 인해 안정적인 원형 궤도, 복잡한 타원 궤도, 또는 혼돈스러운 상호작용이 나타나며, 이는 우주 규모의 동역학을 이해하는 기초입니다.
