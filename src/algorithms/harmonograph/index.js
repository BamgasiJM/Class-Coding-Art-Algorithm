...
export default {
  longDescription: {
    ko: String.raw`
**Harmonograph**(하모노그래프)는 **감쇠하는 사인 진동** 여러 개를 합성하여 정교한 **리사주 곡선**(Lissajous Curve)을 그리는 알고리즘입니다. $x$축과 $y$축에 각각 2개씩, 총 4개의 **진동자(Oscillator)** 를 배치하고 각각 다른 주파수, 위상, 진폭, 감쇠율을 적용합니다. 시간이 경과함에 따라 진폭이 **지수적으로 감소**하며 궤적이 중심부로 점차 수렴하는 아름다운 **나선형(Spiral)** 패턴을 형성하며, 특히 진동자들의 주파수 비율이 정수비(1:2, 2:3, 3:5 등)를 이룰 때 극도로 정교하고 대칭적인 기하학 구조가 나타납니다.
...
- **음악 시각화(Music Visualization):** 음악의 주파수 성분을 리사주 곡선으로 변환하여 음악을 볼 수 있도록.
