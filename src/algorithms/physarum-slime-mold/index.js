...
export default {
  longDescription: {
    ko: String.raw`
**Physarum Polycephalum(황색 점균류) 시뮬레이션**은 **다세포 무척추동물의 최적화 행동**을 **에이전트 기반 모델(Agent-Based Model)**로 구현한 알고리즘입니다. 각 에이전트는 **전방·좌·우 세 방향의 페로몬 농도**를 **센서(Sensor)**로 감지하고, 가장 강한 신호를 따라 이동하며 자신의 궤적을 **트레일(Trail)**로 남깁니다. 트레일은 매 프레임 **확산(Diffusion)** 및 **감쇠(Decay)**되고, 에이전트들은 서로의 화학 신호에 끌려 **자발적으로 최적 연결망(Optimal Network)**을 형성합니다. 이 **창발적 행동(Emergent Behavior)**은 실제 생물학 실험에서 점균류가 미로의 최단 경로를 찾는 현상과 동일한 메커니즘입니다.
