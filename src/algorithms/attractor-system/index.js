...
export default {
  longDescription: {
    ko: String.raw`
**Attractor System**은 중력과 유사한 인력을 발휘하는 하나 이상의 **어트랙터(Attractor)**가 주변의 파티클들을 끌어당기며, 그 궤적이 시간에 따라 복잡한 패턴을 형성하는 물리 기반 동역학 알고리즘입니다. 천체 역학(Celestial Mechanics)에서 출발하여 뉴턴의 만유인력 법칙을 이산 파티클 시스템으로 모델링하며, **정적 어트랙터(Fixed Attractor)** 또는 **동적 어트랙터(Moving Attractor, 예: Lissajous 곡선)**의 상호작용으로부터 우주적 미감과 수학적 우아함을 동시에 드러내는 시각 작품을 생성합니다.
...
4. **속도 감쇠(Damping):** 에너지 소산을 모델링하여 파티클이 무한히 가속되는 것을 방지합니다.
   - $\mathbf{v}_{new} = \mathbf{v}_{old} \cdot \alpha + \mathbf{a} \cdot \Delta t$ (여기서 $\alpha$는 감쇠 계수, $0 < \alpha < 1$)

### 2. 주요 특징 및 장점
- **궤도 역학(Orbital Mechanics):** 파티클들이 어트랙터 주변에서 타원형, 포물선형, 쌍곡선형 궤도를 그리며, 마치 태양 주위를 도는 행성처럼 우주적 아름다움을 표현합니다.
- **다양한 시각적 복잡성:** 여러 어트랙터의 경합으로 인해 혼돈적(Chaotic) 궤적이 발생하며, 파라미터 미세 조정으로 매우 다양한 패턴을 도출할 수 있습니다.
- **현실감 있는 물리 표현:** 역제곱 법칙을 정확히 구현하므로, 관찰자는 실제 중력계에서 일어날 법한 자연스러운 움직임을 경험합니다.
- **상호 작용의 풍부함:** 동적 어트랙터, 정적 장애물, 파티클 간 상호작용 등을 추가할 수 있어 확장성이 높습니다.

### 3. 구현 시 고려 사항
- **거리 계산 최적화:** $d$를 매번 제곱근으로 계산하는 대신, 거리의 제곱 $d^2$를 직접 비교하고 필요한 순간에만 제곱근을 구합니다 ($O(n)$ 최적화).
- **감쇠 계수의 선택:** 감쇠가 작으면 파티클이 계속 회전하고, 크면 빠르게 수렴하여 그림자처럼 어트랙터 중심으로 모입니다. 미적 목표에 따라 세밀한 조절이 필요합니다.
- **다중 어트랙터 관리:** 여러 어트랙터가 있을 때 각각의 힘을 벡터 합으로 통합하며, 각 어트랙터의 강도를 독립적으로 제어할 수 있도록 구현합니다.
- **렌더링 성능:** 파티클 수가 늘어날수록 거리 계산 횟수가 급증하므로, GPU 가속 또는 공간 분할 구조 (Grid) 도입을 고려합니다.

### 4. 활용 분야
- **Generative Art & 시각 설치:** 우주적 미감을 담은 대규모 파티클 아트, 인터랙티브 설치미술에서 마우스나 센서로 어트랙터를 제어하는 작품.
