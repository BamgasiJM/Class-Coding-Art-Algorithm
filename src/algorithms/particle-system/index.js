...
export default {
  longDescription: {
    ko: String.raw`
**Particle System(파티클 시스템)**은 다수의 **개별 입자(Particle)**를 **방출(Emission) → 이동(Movement) → 소멸(Lifespan Decay)**의 생명주기로 관리하는 **동역학 시뮬레이션**의 기본 알고리즘입니다. 각 파티클은 위치, 속도, 가속도, 수명 등의 상태를 가지며, 각 프레임마다 새로운 파티클을 방출하고 기존 파티클을 갱신하며 수명이 다한 파티클을 제거합니다. **중력, 감속(Drag), 풍장(Wind Force)** 등의 물리 효과를 적용하면, 폭발의 불꽃, 연기의 확산, 물의 분사, 불의 타오름 등 극도로 동적이고 자연스러운 시각화를 생성할 수 있으며, 실시간 그래픽스의 가장 근본적이면서도 강력한 도구입니다.
