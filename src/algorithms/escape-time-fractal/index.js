...
export default {
  longDescription: {
    ko: String.raw`
**Escape-Time Fractal(탈출 시간 프랙탈)**은 **복소평면(Complex Plane)**의 각 점 $c$에 대해 **만델브로트 집합**의 생성 함수 $z_{n+1} = z_n^2 + c$ (초기값 $z_0 = 0$)를 반복 적용하고, 절댓값이 **탈출 반경 $R = 2$를 초과**하는 데 걸리는 반복 횟수를 계산하는 **복소수 동역학** 알고리즘입니다. 탈출까지의 반복 횟수를 색상으로 매핑하면, **무한히 복잡한 자기유사 경계(Self-Similar Boundary)**를 갖는 만델브로트 집합의 극도로 정교한 구조가 드러나며, 이는 극히 단순한 수식으로부터 놀라운 기하학적 패턴이 창발하는 현상을 보여줍니다.
...
2. **발산 판정:**
   - 절댓값(Modulus): $|z_n| = \sqrt{z_n^{real}^2 + z_n^{imag}^2}$
