...
export default {
  longDescription: {
    ko: String.raw`
**Elastic Collision**(완전 탄성 충돌)은 두 원이 **접촉하는 순간을 거리 기반으로 감지**하고, **운동량**과 **운동 에너지** 모두 보존하여 반발 속도를 계산하는 물리 기반 알고리즘입니다. 충돌 법선 방향(두 원의 중심을 잇는 직선)에서만 속도 성분이 교환되며, 접선 방향(법선에 수직)의 속도는 변하지 않습니다. 질량비에 따라 작은 공은 크게 튕기고 큰 공은 적게 움직이며, **관통 해제(Penetration Resolution)** 기법으로 원들의 겹침을 방지하여 당구공 같은 자연스러운 반발 움직임을 재현합니다.
...
### 4. 활용 분야
- **물리 기반 게임:** 당구, 탁구, 야구 등 구체적인 충돌이 중요한 게임의 핵심 메커니즘.
