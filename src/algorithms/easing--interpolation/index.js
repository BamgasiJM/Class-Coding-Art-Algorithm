...
export default {
  longDescription: {
    ko: String.raw`
**Easing & Interpolation**(이징과 보간)은 시작점 $S$에서 끝점 $E$까지의 움직임을 **시간에 따라 부드럽고 자연스럽게** 표현하는 애니메이션의 수학적 기법입니다. **선형 보간(Linear Interpolation, Lerp)** 은 일정한 속도로 움직이는 단순한 중간값 계산을 제공하고, **이징(Easing)** 은 속도 곡선을 비선형으로 변형하여 가속도, 감속도, 반동 등 현실 세계의 자연스러운 움직임을 표현합니다. 게임, UI 애니메이션, 모션그래픽, 데이터 시각화 등 모든 실시간 그래픽 시스템에서 핵심적으로 사용되며, 제너레이티브 아트에서도 표현력 있는 움직임을 창출하는 기초 도구입니다.
