...
export default {
  longDescription: {
    ...
    ko: String.raw`
**Cellular Automata**(세포 자동자, CA)는 이산 격자(바둑판) 위의 각 칸이 몇 가지 상태 중 하나를 가지며, **주변 칸들의 상태를 바탕으로** 정해진 약속에 따라 **동시에 상태를 바꾸는** 시스템입니다. 오토마타 이론에서 비롯된 이 개념은 Stephen Wolfram의 연구와 John Conway의 Game of Life로 대중화되었으며, 매우 단순한 국소 규칙으로부터 프랙탈, 패턴, 복잡성이 어떻게 스스로 생겨나는지를 보여주는 수학적 모델로서 생물학, 물리학, 컴퓨터 과학에서 널리 쓰입니다.
...
4. **경계 조건:** 격자의 끝에서 이웃을 정의하는 방식
   - **Open Boundary:** 격자 외부를 0으로 간주
   - **Periodic Boundary:** 양 끝을 연결하여 순환 구조 형성

### 2. 주요 특징 및 장점
- **단순한 규칙:** 규칙이 단순하면서도 (겨우 8가지 경우의 수), 복잡한 패턴이 발생하는 것이 세포 자동자의 핵심 매력입니다.
- **결정론적이면서도 비예측성:** 규칙이 명확하므로 완전히 결정론적이지만, 초기 조건의 미세한 변화가 극적으로 다른 결과를 낳는 특성을 보입니다.
- **프랙탈 구조:** Rule 90은 특히 **시에르핀스키 삼각형(Sierpinski Triangle)**과 정확히 동일한 프랙탈 기하학을 생성하며, 자기 유사성(Self-similarity)과 무한 정교함을 보여줍니다.
- **효율적인 계산:** 각 세대는 $O(n)$ 시간에 계산되므로 매우 빠른 시뮬레이션이 가능합니다.
...
- **렌더링 최적화:** 세대 수가 크면 화면에 맞추기 위해 셀 크기를 조절하거나 대규모 그리드를 구현합니다.
...
- **암호학:** 규칙 기반 난수 생성기, 선형 피드백 시프트 레지스터(LFSR) 기반 암호화.
  ...
  {
    key: 'ruleNumber',
    label: 'Rule 번호 : 22,50,54,77,90,129,183,246',
    min: 0,
    max: 255,
    step: 1,
    default: 90,
