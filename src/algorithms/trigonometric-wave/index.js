...
export default {
  longDescription: {
    ko: String.raw`
**Trigonometric Wave(삼각함수 파형)**은 **사인(Sine)과 코사인(Cosine) 함수의 주기성**을 이용하여 **다양한 파형과 리듬을 생성**하는 **제너레이티브 아트의 가장 기본적인 알고리즘** 중 하나입니다. **서로 다른 진폭(Amplitude), 주파수(Frequency), 위상(Phase)**을 가진 **여러 삼각함수를 조합**하면, **단순한 곡선이 극도로 복잡하고 유기적인 리듬**을 가진 형태로 변화합니다. 이 원리는 **오디오 신호 처리, 진동 시뮬레이션, 물결 표현, 데이터 시각화, 음악 합성** 등 **광범위한 분야**에서 활용됩니다. **극히 단순한 수식의 작은 변화**만으로도 **전혀 다른 시각적 결과**를 만들어내므로, **수학과 예술의 완벽한 교집합**을 보여주는 대표 예제입니다.
