...
export default {
  longDescription: {
    ko: String.raw`
**L-System(Lindenmayer System, 린덴마이어 시스템)**은 생물의 **성장 과정과 분화 규칙**을 **형식 언어와 문자열 재작성(String Rewriting)**으로 모델링하는 **절차적 생성 알고리즘**입니다. 초기 문자열(공리, Axiom)에 **생산 규칙(Production Rules)**을 반복적으로 적용하여 점진적으로 복잡해지는 문자열을 생성하고, 이를 **Turtle Graphics**(거북이 그래픽, 벡터 지시어 기반 그리기)로 해석하면 나무, 식물, 고사리, 산호, 꽃 등 극도로 자연스러운 **분기 구조(Branching Structure)**를 자동 생성할 수 있습니다. 단순한 문법 규칙만으로도 자기유사적이고 생명력 있는 형태가 창발하므로, 프랙탈과 식물 모델링에서 가장 대표적으로 사용됩니다.
