...
export default {
  longDescription: {
    ko: String.raw`
**Random Walk**(무작위 보행)는 시작점에서 매 단계마다 **무작위 방향**으로 일정 거리만큼 이동하는 **점의 궤적을 모델링**하는 알고리즘입니다. 단순하고 국소적인 운동 규칙만으로도 **브라운 운동(Brownian Motion)**, **확산(Diffusion)**, **포식자-피식자 추적**, **도시 구조의 거리망** 등 자연계의 우연과 질서가 얽혀있는 현상을 근본적으로 설명합니다. 여러 독립적인 워커들이 동시에 보행할 때, 겹쳐진 궤적들은 개별 무작위성이 모여 **집단 패턴(Collective Pattern)** 을 형성하는 모습을 보여줍니다.
...
3. **확률 분포:**
   - $n$번 스텝 후, 원점으로부터 거리 $d$에 있을 확률 $P_n(d)$는 **정규분포** 에 수렴 (중심극한정리)
