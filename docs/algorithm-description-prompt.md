# 알고리즘 설명글(한국어) 생성 프롬프트

다른 LLM에 알고리즘 상세 페이지의 **한국어 설명(`longDescription.ko`)** 을 맡길 때 쓰는 프롬프트다.
영어 설명(`longDescription.en`)은 별도로 유지하므로 이 프롬프트는 **한국어만** 생성한다.
아래 블록을 복사해 `<<알고리즘 이름>>` 부분만 바꿔서 사용한다. 결과물은
`src/algorithms/<slug>/index.js`의 `ko: String.raw`...`` 백틱 사이에 그대로 붙여넣는다.
...
"한국어 설명글"을 작성합니다. 결과물은 JS 파일의 String.raw`...` 백틱 사이에
...
## 출력 형식
한국어 마크다운 본문 "하나만" 출력하세요.
코드 펜스, 제목 라벨([KO] 등), 설명, 인사말을 앞뒤에 붙이지 마세요.
출력의 첫 글자가 곧바로 설명 도입 문단이어야 합니다. (영어 설명은 만들지 않습니다.)
...
받은 한국어 마크다운을 `src/algorithms/<slug>/index.js`의 `ko` 필드에만 붙여넣는다.
`en`은 기존에 작성된 것을 그대로 둔다.
...
export default {
  longDescription: {
    ko: String.raw`
(여기에 생성된 한국어 마크다운 붙여넣기)
`,
    en: String.raw`...`,  // 기존 영어 설명 유지
  },
  sketch,
...
`\vec` 등)를 JS 이스케이프로 먹어치워 수식을 조용히 깨뜨린다(KaTeX가 빨간색 에러로 렌더링).
